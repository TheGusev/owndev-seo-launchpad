              ...
              <div key={i} className="flex items-start gap-2 py-1.5 border-b border-border/30">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {link.status && <span className={`text-[10px] ${link.ok ? "text-muted-foreground" : "text-destructive font-bold"}`}>{link.status}</span>}
                  </div>
                  {link.text && <p className="text-xs text-muted-foreground truncate">{link.text}</p>}
                </div>
              </div>
            ))}
          </div>

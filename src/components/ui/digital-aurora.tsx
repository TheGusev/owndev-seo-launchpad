  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main() {
        ...
        // Brand colors
        vec3 color1 = vec3(0.24, 0.85, 0.76);  // Turquoise #3dd9c3
        vec3 color2 = vec3(0.39, 0.40, 0.95);  // Blue #6366f1
        vec3 color3 = vec3(0.55, 0.36, 0.96);  // Purple #8b5cf6
        ...
        for (int i=0; i<MARCH_STEPS; i++) {
            ...
            if (density > 0.0) {
                vec3 auroraColor = mix(
                  mix(color1, color2, sin(iTime * 0.3 + p.y * 1.5) * 0.5 + 0.5),
                  color3,
                  cos(iTime * 0.2 + p.x * 2.0) * 0.5 + 0.5
                );
...
export const AuroraBackground: React.FC<AuroraBackgroundProps> = ({ children }) => {
  ...
  return (
    <div className="relative min-h-screen">
      {/* Aurora shader - fixed position behind everything */}
      {webglSupported && (
        <div className="fixed inset-0 z-0">
          <InteractiveShader
            flowSpeed={reducedMotion ? 0.1 : 0.3}
            colorIntensity={0.8}
            noiseLayers={reducedMotion ? 2 : 5}
            mouseInfluence={0.2}
          />
        </div>
      )}
      ...
      <div className="fixed inset-0 z-[1] bg-gradient-to-b from-background/70 via-background/50 to-background/80" />

/**
 * P0-словарь — человекочитаемые расшифровки критических preflight-фейлов.
 *
 * Зачем:
 *  В отчётах PRO раньше выводились голые коды вроде «AILLM_P0_FAQ_BLOCK» —
 *  пользователь не понимал, что это и что с этим делать. Теперь к каждому
 *  коду есть title / why / whatToDo на русском.
 *
 * Источники кодов:
 *  - owndev-backend/src/services/preflight/ruleEngine.ts (SEO/DIRECT/SCHEMA/AILLM)
 *  - owndev-backend/src/services/preflight/v1Guardrails.ts (V1_P0_*)
 *
 * Используется:
 *  - PDF/Word PRO-отчёт на фронте (через API result.preflight_rollup.failed_p0_codes)
 *  - JSON-pack developerPack, чтобы AI-конструкторы (Lovable/Cursor/v0)
 *    могли цитировать рекомендации словами, а не кодами.
 */
...
export const P0_DICTIONARY: Record<string, P0Explanation> = {
  ...
  SEO_P0_H1_PRESENT: {
    ...
    why: 'H1 определяет основную тему страницы для поисковика и пользователя. Без него ранжирование падает, а CR — снижается.',
  ...
  SEO_P0_INDEXABLE: {
    ...
    whatToDo: 'Убрать noindex из meta robots и проверить заголовок X-Robots-Tag в HTTP-ответе. Только utility-страницы (личный кабинет, корзина) должны быть noindex.',
  },
  SEO_P0_CANONICAL: {
  ...
  DIRECT_P0_PRIMARY_CTA: {
    ...
    whatToDo: 'Разместить заметную CTA (Заказать / Получить расчёт / Записаться) в верхней зоне страницы — над первым экраном на мобильном.',
  },
  DIRECT_P0_PHONE_CLICKABLE: {
    ...
    whatToDo: 'Обернуть телефон в <a href="tel:+7XXXXXXXXXX">. Использовать формат E.164 без пробелов в href.',
  },
  DIRECT_P0_LEAD_FORM: {
    title: 'На странице нет формы заявки',
    why: 'Без формы (или хотя бы открытого чата/кнопки заказа) страница не собирает лиды — теряется главный канал конверсии.',
    whatToDo: 'Добавить компактную форму (имя + телефон) либо в основной блок, либо как floating-CTA. Минимум полей, обязательно политика.',
  },

  // ─── SCHEMA P0 ───────────────────────────────────────────────
  SCHEMA_P0_JSONLD_PRESENT: {
    title: 'Нет JSON-LD разметки на странице',
    why: 'Schema.org JSON-LD — стандартный сигнал для поисковиков и AI-ботов. Без него страница не получит rich snippet и хуже цитируется LLM.',
  ...
  SCHEMA_P0_GRAPH_ROOT: {
    ...
    whatToDo: 'Объединить все блоки JSON-LD в один <script> с структурой { "@context": "https://schema.org", "@graph": [...] }.',
  },
  SCHEMA_P0_REQUIRED_TYPES: {
    title: 'В JSON-LD отсутствуют обязательные типы для этой ниши',
    why: 'Каждый тип проекта требует своего набора схем (например LocalBusiness + Service для гео-услуг). Без них rich-результаты не показываются.',
  ...
  AILLM_P0_LLMS_TXT: {
    ...
    why: 'llms.txt — официальный стандарт для AI-агентов (ChatGPT, Perplexity, Claude). Без него ваш сайт реже попадает в AI-ответы.',
    whatToDo: 'Положить /llms.txt в корень с разделами: # Site, # Allow для AI-ботов, # Pages со списком ключевых страниц и кратких описаний.',
  },
  AILLM_P0_AI_ROBOTS: {
    title: 'В robots.txt нет правил для AI-ботов',
    why: 'По умолчанию AI-боты (GPTBot, ClaudeBot, PerplexityBot, YandexGPT и др.) могут быть заблокированы wildcard-правилами — это убирает сайт из AI-выдачи.',
    whatToDo: 'Добавить в /robots.txt секции «User-agent: GPTBot» / «User-agent: PerplexityBot» / «User-agent: ClaudeBot» с «Allow: /» (либо явные ограничения по контенту).',
  },
  AILLM_P0_INTRO_ANSWER: {
    title: 'Нет краткого ответа в первом абзаце страницы',
    why: 'AI-ассистенты ищут прямой ответ в первых 200 словах. Без него сайт не попадает в AI-цитаты — даже если контент дальше отличный.',
    whatToDo: 'Первый параграф каждой ключевой страницы — прямой 40–80-словный ответ на основной запрос. Без воды, в первом предложении ответ.',
  },
  AILLM_P0_FAQ_BLOCK: {
  ...
  V1_P0_UTILITY_NO_SITEMAP: {
    ...
    why: 'Sitemap должен содержать только индексируемые страницы. Лишние URL сбивают поисковик с приоритетов.',
...
/**
 * Получает развёрнутое объяснение P0-кода. Для неизвестного кода возвращает
 * fallback с самим кодом — это не должно случаться, но защищает PDF/Word
 * от пустых строк.
 */
export function explainP0Code(code: string): P0Explanation {
  ...
  return {
    ...
    whatToDo: 'Свериться с правилами preflight в owndev-backend/src/services/preflight/ruleEngine.ts.',
  };
}

/**
 * Форматирует один P0-код в одну строку для текстовых отчётов:
 *   «<КОД>: <title> · <why> · Что делать: <whatToDo>».
 * Используется в PDF/Word/MD-сериализаторах developerPack.
 */
export function formatP0OneLiner(code: string): string {
  const e = explainP0Code(code);
  return `${code}: ${e.title} · ${e.why} · Что делать: ${e.whatToDo}`;
}

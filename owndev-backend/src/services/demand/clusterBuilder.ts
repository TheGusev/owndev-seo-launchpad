 ...
 *
 * ВАЖНО: список НЕ содержит дубликатов и пар «стем-приставка»:
 *   'красн' уже покрывает 'красно' через startsWith()
 *   'омск'  уже покрывает 'омске'
 *   'перм'  уже покрывает 'перми'
 * Поэтому для каждого города оставлен ровно один минимальный стем.
 * 'красн' умышленно «жадный» — ловит и Красноярск, и Краснодар, и Краснокамск,
 * это нормально: гео-разделение происходит на уровне geoTargeter по region_code.
 */
const GEO_STEMS = new Set([
  // миллионники
  'москв', 'спб', 'питер', 'екате', 'новос', 'нижн', 'казан', 'самар',
  'омск', 'челяб', 'росто', 'уфа', 'красн', 'воронеж', 'перм', 'волгог',
  // крупные региональные центры
  'сочи', 'тюмен', 'тольят', 'ижевск', 'барнаул', 'ульян', 'иркут', 'хабар',
  'ярос', 'махач', 'оренб', 'кемер', 'рязан', 'тула', 'липецк', 'киров',
  'чебок', 'калин', 'брянск', 'курск', 'иванов', 'магни', 'тверь', 'ставр',
  'белго', 'архан', 'влади', 'смолен', 'калуг', 'орёл', 'орел', 'мурман',
  'нальч', 'таган', 'симфе', 'ялта', 'волог',
  // регионы / страны
  'крым', 'россия', 'росси', 'рф',
]);
